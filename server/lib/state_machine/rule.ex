defmodule StateMachine.Rule do
  @moduledoc """
  Choice state rule.
  """
  defstruct [:next, :rule]
    ...
    with {:ok, variable_fn} <- path_value(variable, result_type: :value_path) do
      rule = fn args ->
        case variable_fn.(args) do
          {nil, ""} -> false # returned nil because the value is not present
          {nil, _} -> true # returned nil because the value is null
          {_, _} -> false # value not null
        end
      end
      {:ok, rule}
    end
